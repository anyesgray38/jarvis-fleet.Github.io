...
class FleetNode:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"node_id": self.node_id, "address": self.address, "network": self.network,
                "role": self.role, "trust": self.trust, "status": self.status,
                "capabilities": sorted(self.capabilities), "modalities": sorted(self.modalities),
                "labels": sorted(self.labels), "metadata": dict(self.metadata)}
