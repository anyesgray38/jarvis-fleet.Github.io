...
class FleetScheduler:
    ...
    def rank(self, workload: Workload) -> list[NodeScore]:
        ...
        for node in self.nodes:
            if node.status not in {"ready", "connected", "healthy"}:
                continue
            if not node.supports(workload.capability):
                continue
            if not workload.required_labels.issubset(node.labels):
                continue
            if not workload.required_modalities.issubset(node.modalities):
                continue
            try:
            ...
            score += len(workload.required_labels.intersection(node.labels)) * 10.0
            score += len(workload.required_modalities.intersection(node.modalities)) * 5.0
            if node.network == "tailscale":
                score += 2.0
                reasons.append("private overlay")
            if node.trust == "verified":
                score += 5.0
                reasons.append("verified node")
