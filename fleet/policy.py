...
class NetworkPolicy:
    """Fail-closed network admission independent of the overlay provider."""
    ...
    def authorize(self, request: NetworkRequest, *, node: Any | None = None) -> None:
        ...
        if node is not None and str(getattr(node, "trust", "untrusted")) not in {"trusted", "verified"}:
    ...
    def from_dict(data: dict[str, Any]) -> "NetworkPolicy":
        return NetworkPolicy(
            ...
            approved_exit_nodes={str(x) for x in data.get("approved_exit_nodes", [])},
        )
