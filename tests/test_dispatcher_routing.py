...
def _components(tmp_path):
    ...
    registry_path.write_text(json.dumps({"capabilities": [
        ...
        {"id": "security.configuration_review", "provider": "aegis", "kind": "security", "tags": ["security", "configuration", "review"], "verification": ["evidence"]}
    ]}))
...
def test_explicit_capability_bypasses_router(tmp_path):
    ...
    result = dispatcher.dispatch({"task_id": "r-2", "capability": "core.website_generation", "objective": "security configuration review", "verification": {"required": False}}, security=_security())
    assert result.status == "passed"
