...
def _app_files(name: str, title: str, description: str, accent: str) -> dict[str, str]:
...
.app-shell {{ min-height:100vh; display:grid; place-items:center; padding:24px; }} .app-card {{ width:min(100%,620px); padding:clamp(28px,7vw,64px); border:1px solid rgba(23,32,42,.12); border-radius:24px; background:rgba(255,255,255,.86); box-shadow:0 22px 70px rgba(42,66,84,.16); }}
...
''',
        ...
        "README.md": f'''# {name}\n\nGenerated by the AEGIS autonomous app builder.\n\n{title}\n\n{description}\n\nThis is a dependency-free installable web-app starter with local browser storage. It can be extended by a governed coding task and verified with the same build/test stages.\n''',
    }
...
def project_create(context: ActionContext, args: dict[str, Any]) -> dict[str, Any]:
    ...
    files = _app_files(name, title, description, accent)
    ...
    return {"project_dir": str(project.relative_to(context.workspace.resolve())), "files": sorted(files), "total_bytes": total, "overwrote": existed, "kind": kind, "stage": "create"}
...
def builder_run(context: ActionContext, args: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"status": "PASS", "kind": created["kind"], "project_dir": created["project_dir"], "stages": [created, built, tested], "evidence": {"files": built["files"], "checks": tested["checks"]}}
