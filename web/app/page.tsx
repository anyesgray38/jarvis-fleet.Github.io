...
const nav = ['Homelab', 'Shark Ops', 'Overview', 'Chat', 'Tasks', 'Agents', 'MCP Capabilities', 'Models', 'Memory', 'Security', 'Pentest Chat', 'Pentest', 'Verification', 'Evidence', 'Settings']
...
export default function Home() {
  ...
  return <div className="shell"><aside className="sidebar"><div className="brand">AEGIS<span>AUTONOMOUS INTELLIGENCE</span></div><nav className="nav" aria-label="AEGIS sections">{nav.map(n => <button type="button" key={n} className={active === n ? 'active' : ''} aria-current={active === n ? 'page' : undefined} onClick={() => { setActive(n); setActionMessage(null) }}>{n}</button>)}</nav><div className="footer">CONTROL PLANE v1.1<br />{data.connected ? 'UPSTREAM CONNECTED' : 'UPSTREAM OFFLINE'}</div></aside><main className="main"><header className="top"><div><div className="eyebrow">{active}</div><div className="title">AEGIS Control Center</div><div className="muted">Autonomous Execution, Governance & Intelligence System</div></div><div className="status" role="status" aria-live="polite"><i className={data.connected ? 'dot' : 'dot off'} />{data.connected ? 'Systems connected' : 'Gateway disconnected'}<button type="button" className="refresh" onClick={() => void refresh()} disabled={refreshing}>{refreshing ? 'Syncing…' : 'Refresh'}</button></div></header>{!data.connected && <div className="notice" role="alert">{data.error || 'Connect the dashboard to the authenticated AEGIS gateway on the Linux host.'}</div>}{actionMessage && <div className={actionMessage.includes('accepted') ? 'notice success' : 'notice'}>{actionMessage}</div>}
      ...
      {active === 'MCP Capabilities' && <CapabilityPanel />}{active === 'Models' && <ModelsPanel />}{active === 'Memory' && <MemoryPanel />}{active === 'Security' && <SecurityPanel />}
      {active === 'Pentest Chat' && <PentestChat />}{active === 'Pentest' && <PentestConsole />}{active === 'Verification' && <VerificationPanel jobs={data.jobs} />}{active === 'Evidence' && <EvidencePanel jobs={data.jobs} />}{active === 'Settings' && <SettingsPanel connected={data.connected} />}
...
function Overview({ data, activeJobs, liveAgents, recentJobs, loading }: { data: State; activeJobs: number; liveAgents: number; recentJobs: Job[]; loading: boolean }) { return <section className="grid"><Metric label="ACTIVE TASKS" value={String(activeJobs).padStart(2, '0')} note="live orchestrator jobs" /><Metric label="FLEET NODES" value={String(liveAgents).padStart(2, '0')} note="connected agents" /><Metric label="JOBS RECORDED" value={String(data.jobs.length)} note="from orchestrator queue" /><Metric label="LAST SYNC" value={data.fetchedAt ? new Date(data.fetchedAt).toLocaleTimeString() : loading ? '…' : '—'} note="5 second polling" small /><div className="card wide"><div className="eyebrow">Execution pipeline</div><div className="pipeline">{pipeline.map((p, i) => <span key={p} className="pipeline-item"><div className="step"><b>{p}</b><small>{i < 2 ? 'governed' : i === 2 ? 'selected' : 'pending'}</small></div>{i < pipeline.length - 1 && <span className="arrow">→</span>}</span>)}</div></div><div className="card wide"><div className="eyebrow">Live task graph</div><div style={{ marginTop: 12 }}>{recentJobs.length ? recentJobs.map(j => <JobRow key={j.id} job={j} />) : <div className="empty">No orchestrator jobs available.</div>}</div></div><div className="card"><div className="eyebrow">Fleet</div><div className="fleet" style={{ marginTop: 12 }}>{data.agents.length ? data.agents.map(a => <AgentRow key={a.id} agent={a} />) : <div className="empty">No agents connected.</div>}</div></div><div className="card"><div className="eyebrow">Governance</div><div className="row"><span>Policy engine</span><span className="badge">ENFORCING</span></div><div className="row"><span>Security admission</span><span className="badge">PASS</span></div><div className="row"><span>Self-audit</span><span className="badge">ACTIVE</span></div><div className="row"><span>Evidence chain</span><span className="badge">SEALED</span></div></div></section> }
...
function ModelsPanel() { return <section className="grid"><InfoCard title="Model Fabric" text="AEGIS selects approved local models through a provider-neutral registry and policy-aware router." items={['LM Studio / LocalAI adapters', 'Text / vision routing', 'Purpose-aware admission', 'Local-only by default']} /><InfoCard title="Runtime controls" text="Model calls are normalized, timed, and recorded so the control center can expose how a response was produced." items={['Task constraints', 'Deterministic ranking', 'Fallback providers', 'Inference evidence']} /></section> }
...
function EvidencePanel({ jobs }: { jobs: Job[] }) { return <section className="grid"><InfoCard title="Evidence chain" text="The dashboard exposes execution history from the orchestrator. Inference requests are also recorded by the model runtime." items={['Task identity', 'Agent identity', 'Inference request ID', 'Verification status']} /><div className="card"><div className="eyebrow">Execution history</div>{jobs.length ? jobs.slice().reverse().map(j => <div className="row" key={j.id}><span>#{j.id} · {j.hostname}</span><span className="muted">{new Date(j.created_at).toLocaleString()}</span></div>) : <div className="empty">No evidence events yet.</div>}</div></section> }
function SettingsPanel({ connected }: { connected: boolean }) { return <section className="grid"><InfoCard title="Runtime" text="The dashboard runs on the private Tailscale network with the orchestrator reachable only from the server process." items={[connected ? 'Orchestrator connected' : 'Orchestrator offline', '5 second telemetry polling', 'Server-side upstream proxy', 'No browser credentials']} /><InfoCard title="Deployment" text="The control center runs directly on the AEGIS Linux host and is reached through the private Tailscale network." items={['Next.js production server', 'Tailscale access', 'Local process supervision', 'Private network only']} /></section> }
