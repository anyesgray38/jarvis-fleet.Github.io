...
type Business = { business_id: string; business_name: string; address: string; phone: string; category: string; website: string; website_state: string; location_class: string; classification: string; opportunity_score: number; score_reasons: string[]; opportunities: string[]; landing_page?: { build?: { status?: string }; project_dir?: string }; evidence: { source: string; observation: string; confidence: string }[] }
type Scan = { scan_id: string; request: { target: string }; summary: { businesses_discovered: number; high_opportunity: number; landing_pages_generated: number; errors: number }; businesses: Business[]; errors: string[]; completed_at: string }
...
export default function ProspectingDashboard() {
  ...
  const refresh = useCallback(async () => {
    ...
    if (businesses.length) setScan(current => current ? { ...current, businesses } : { scan_id: 'stored-prospects', request: { target: 'Stored prospects' }, summary: { businesses_discovered: businesses.length, high_opportunity: businesses.filter(item => item.opportunity_score >= 60 && item.classification !== 'UNKNOWN').length, landing_pages_generated: businesses.filter(item => item.landing_page?.build?.status === 'PASS').length, errors: 0 }, businesses, errors: [], completed_at: '' })
  }, [])
  ...
  return <section className="prospecting-page">
    ...
    {scan && <><div className="prospecting-summary"><Metric label="Businesses discovered" value={scan.summary.businesses_discovered} /><Metric label="High opportunity" value={scan.summary.high_opportunity} /><Metric label="Demos generated" value={scan.summary.landing_pages_generated} /><Metric label="Workflow errors" value={scan.summary.errors} /></div><div className="prospect-list">{scan.businesses.map(business => <ProspectCard key={business.business_id} business={business} />)}</div>{scan.errors.length > 0 && <div className="card wide"><div className="eyebrow">Workflow issues</div>{scan.errors.map(error => <p className="intake-message" key={error}>{error}</p>)}</div>}</>}
...
function ProspectCard({ business }: { business: Business }) {
  return <article className="prospect-card"><div className="prospect-card-top"><div><div className="eyebrow">{business.location_class.replaceAll('_', ' ')}</div><h2>{business.business_name}</h2><p>{business.address || 'Address not verified'}{business.phone ? ` · ${business.phone}` : ''}</p></div><div className="opportunity-score"><strong>{business.opportunity_score}</strong><span>digital opportunity</span></div></div><div className="prospect-tags"><span>{business.classification.replaceAll('_', ' ')}</span><span>Website: {business.website_state}</span>{business.category && <span>{business.category}</span>}</div><div className="prospect-reasons">{business.score_reasons.length ? business.score_reasons.slice(0, 4).map(reason => <p key={reason}>+ {reason}</p>) : <p>No scored gap observed in the bounded audit.</p>}</div><div className="prospect-footer"><span>{business.evidence.length} evidence record(s)</span><span>{business.landing_page?.build?.status === 'PASS' ? 'Static preview verified' : business.landing_page ? 'Demo needs review' : 'No demo generated'}</span></div></article>
}
